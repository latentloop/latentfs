...
# Test multiple full snapshots with restore to different points.
#
# Usage: bash tests/scripts/test_save_restore_full_1.sh <source-folder>
#
# Debugging:
...
#   - Daemon logs are copied to tests/log/save_restore_full_1_trace.log
#   - Use LATENTFS_DAEMON_LOG to customize daemon log location (default: ~/.latentfs/daemon.log)
#
# Test Flow:
#   1. Full-fork source folder
...
cleanup() {
    ...
    if [ -f "$DAEMON_LOG" ]; then
        echo "  Daemon log -> $LOG_DIR/save_restore_full_1_trace.log"
        cp "$DAEMON_LOG" "$LOG_DIR/save_restore_full_1_trace.log" 2>/dev/null || true
    fi
...
SNAPSHOT_TAG_1="test-full-multi-snap1-$$"
...
SNAPSHOT_TAG_2="test-full-multi-snap2-$$"
...
SNAPSHOT_TAG_3="test-full-multi-snap3-$$"
