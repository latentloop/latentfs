...
# Test save/restore overlay snapshot on a soft-forked NFS mount.
#
# Usage: bash tests/scripts/test_save_restore_overlay_0.sh <source-folder>
#
# Debugging:
...
#   - Daemon logs are copied to tests/log/save_restore_overlay_trace.log
...
cleanup() {
    ...
    if [ -f "$DAEMON_LOG" ]; then
        cp "$DAEMON_LOG" "$LOG_DIR/save_restore_overlay_trace.log"
        echo "  Daemon log -> $LOG_DIR/save_restore_overlay_trace.log"
    fi
...
# --- 4. Save overlay snapshot ---
echo "=== Step 4: Saving overlay snapshot ==="
SNAPSHOT_TAG="test-overlay-$(date +%s)"
"$BIN" data save -d "$DATA_FILE" -m "Test overlay snapshot" -t "$SNAPSHOT_TAG"
echo ""
...
echo ""
echo "=== PASSED ==="
echo "  Overlay save/restore test completed successfully."
echo "  - Created overlay fork from: $SOURCE_FOLDER"
echo "  - Saved overlay snapshot: $SNAPSHOT_TAG"
