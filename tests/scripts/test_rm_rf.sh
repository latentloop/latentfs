...
# Test rm -rf performance on a soft-forked NFS mount.
#
# Usage: bash tests/scripts/test_rm_rf.sh <source-folder>
#
# Steps:
...
#   5. Time rm -rf on that directory
...
TMP_DIR=$(mktemp -d -t latentfs_rmrf_test)
...
cleanup() {
    ...
    if [ -f "$DAEMON_LOG" ]; then
        cp "$DAEMON_LOG" "$LOG_DIR/rm_rf_trace.log"
        echo "  Daemon log → $LOG_DIR/rm_rf_trace.log"
    fi
...
# --- 5. Time rm -rf ---
echo "=== Step 5: rm -rf $largest_name ==="
echo "  Path: $largest_dir"
echo "  Files: $largest_count"
echo ""

start_ts=$(perl -MTime::HiRes=time -e 'printf "%.3f\n", time')
rm -rf "$largest_dir" 2>&1
...
# Check for errors
if [ -d "$largest_dir" ]; then
    echo ""
    echo "WARNING: Directory still exists after rm -rf!"
    remaining=$(find "$largest_dir" -type f 2>/dev/null | wc -l | tr -d ' ')
